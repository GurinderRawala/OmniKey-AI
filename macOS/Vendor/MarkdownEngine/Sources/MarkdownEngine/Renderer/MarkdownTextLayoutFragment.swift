...
final class MarkdownTextLayoutFragment: NSTextLayoutFragment {
    ...
    private func drawCodeBlockBackground(at point: CGPoint, in context: CGContext) {
        ...
        guard let color = ts.attribute(
            .backgroundColor,
            at: range.location,
            longestEffectiveRange: &codeBlockRange,
        ...
        color.setFill()
        // A multiline block is split into multiple TextKit fragments. Round
        // only the outer edge fragments; middle fragments remain square and
        // join them into one continuous card.
        let isFirstFragment = range.location <= codeBlockRange.location
        let isLastFragment = NSMaxRange(range) >= NSMaxRange(codeBlockRange)
        let blockPath = codeBlockBackgroundPath(
            in: bgRect,
            roundTop: isFirstFragment,
            roundBottom: isLastFragment
        )
    ...
    private func codeBlockBackgroundPath(
        in rect: CGRect,
        roundTop: Bool,
        roundBottom: Bool
    ) -> NSBezierPath {
        let radius = min(8, rect.width / 2, rect.height / 2)
        guard radius > 0, roundTop || roundBottom else { return NSBezierPath(rect: rect) }

        let path = NSBezierPath()
        path.move(to: CGPoint(x: rect.minX, y: roundTop ? rect.minY + radius : rect.minY))
        if roundTop {
            path.curve(
        ...
        path.line(to: CGPoint(x: rect.maxX - (roundTop ? radius : 0), y: rect.minY))
        if roundTop {
            path.curve(
        ...
        path.line(to: CGPoint(x: rect.maxX, y: rect.maxY - (roundBottom ? radius : 0)))
        if roundBottom {
            path.curve(
        ...
        path.line(to: CGPoint(x: rect.minX + (roundBottom ? radius : 0), y: rect.maxY))
        if roundBottom {
            path.curve(
