...
/// The Terms & Conditions bundled with the desktop app. Kept in sync with
/// the top-level `TERMS.md` file — update `currentVersion` whenever the
/// text changes materially so previously-accepting users are re-prompted
/// on next launch.
enum TermsContent {
    /// Bump whenever `text` (or `TERMS.md`) is updated materially.
    /// Format: date of the terms revision.
    static let currentVersion = "2026-01-01"

    /// Full Markdown source of the current Terms & Conditions.
    /// Mirrors `TERMS.md` at the repository root.
    static let text: String = """
# OmniKey AI — Terms and Conditions
